...
Jede Weitergabe erhält eine Kopfzeile **Nachricht von „Chatname“** mit dem aktuellen Namen des Quellchats. Der Antworttext bleibt in einem gekennzeichneten Inhaltsblock unverändert. Davor steht eine Kommunikationsregel: ausschließlich im empfangenden Chat antworten, keine direkten Rücksendungen oder Zwischenmeldungen an andere Chats. Beim Kopieren allein wird nichts ergänzt.

Bei einer Weitergabe wird der tatsächliche Quellchat als Absenderkontext verwendet: im Loop der Chat, dessen Antwort weitergereicht wird, beim manuellen Einfügen der kopierte Quellchat. Der technische Einrichtungskontext dient nicht als Absender der Nachrichten. Die Übergaberegel erklärt frühere direkte Rücksendeanweisungen für die aktuelle Übergabe ausdrücklich für nicht anwendbar. Sie ist eine Anweisung, keine technische Deaktivierung der Codex-Werkzeuge. Der Loop prüft die letzten Turn-IDs der übrigen Teilnehmer und hält bei unerwarteten Aufträgen an; bereits gesendete Nachrichten kann er nicht zurücknehmen.
...
### Verbindung nach Codex-Updates
Bei fehlgeschlagenen Einzelchat-Statusabfragen sucht Workflow Loop erneut nach einer aktuellen lokalen Codex-Verbindung. Eine Ersatzadresse wird erst übernommen, wenn die App genau die angefragte Chat-ID bestätigt. Fehlende Runtime-Dateien werden wie bisher neu aufgelöst. Sendungen werden niemals automatisch wiederholt. Ist keine Verbindung verifizierbar, bleibt der Versand angehalten; ein abgeschlossener Lauf startet durch die Wiederherstellung nicht selbstständig. Die Erkennung der lokalen Codex-Pipes ist versionsabhängig und kann bei grundlegend geänderten App-Schnittstellen eine Anpassung benötigen.
