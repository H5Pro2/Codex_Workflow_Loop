(()=>{
 ...
 let timer,inflight=false,signature='';
 ...
 async function refresh(){
  clearTimeout(timer);if(!panel.open||inflight)return;inflight=true;
  try{
   const data=await read();feedback.textContent=data.error||`${data.entries.length} / ${data.limit} Einträge · neueste zuerst`;
 ...
 document.getElementById('debug-clear').onclick=async()=>{
  try{const r=await fetch('/api/command',{method:'POST',headers:{'Content-Type':'application/json','X-Workflow-Loop':'1'},body:JSON.stringify({action:'clear_debug'}),signal:AbortSignal.timeout(5000)});if(!r.ok)throw Error('Leeren fehlgeschlagen');signature='';await refresh();}catch(e){feedback.textContent=e.message;}
 };
